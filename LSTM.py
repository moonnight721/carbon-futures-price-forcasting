...
for dataset_name, features in feature_sets.items():
    ...
    for t in range(start, end):
        ...
        for i in range(loop_times):
            ...
            x=LSTM(units,activation='tanh',return_sequences=False)(inputs)
            # x=LSTM(24,activation='tanh',return_sequences=False)(x)
            x = tf.keras.layers.ELU()(x)
            # x = tf.keras.layers.ReLU()(x)
            # x=Dropout(dropout_rate)(x)

            # x=LSTM(128,activation='elu')(x)
            # x=Dropout(dropout_rate)(x)
            ...
            print(f"Loop {i+1}/{loop_times} complete.")
