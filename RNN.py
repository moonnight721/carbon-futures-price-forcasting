...
from tensorflow.keras.layers import Input, SimpleRNN, Dropout, Dense, LeakyReLU
...
for dataset_name, features in feature_sets.items():
    ...
    for t in range(start, end):
        ...
        for i in range(loop_times):
            ...
            # 第一層 RNN
            x = SimpleRNN(units,activation='tanh',return_sequences=False)(inputs)
            # x = LeakyReLU(alpha=0.05)(x)
            # x = Dropout(dropout_rate)(x)
            # # 第二層 RNN
            # x = SimpleRNN(128,activation='elu')(x)
            # # x = LeakyReLU(alpha=0.05)(x)
            # x = Dropout(dropout_rate)(x)
            x = Dense(24,activation='elu')(x)
            # x = LeakyReLU(alpha=0.05)(x)
