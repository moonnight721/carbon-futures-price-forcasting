...
from tensorflow.keras.layers import GRU, Conv1D,concatenate, Reshape,BatchNormalization,GlobalAveragePooling1D,Dropout,Input,Dense,MaxPooling1D
...
for dataset_name, features in feature_sets.items():
    ...
    for t in range(start, end):
        ...
        for i in range(loop_times):
            ...
            GRU_layer = GRU(units, activation='tanh',  return_sequences=False, name='GRU_layer')(x)
            # x = tf.keras.layers.LeakyReLU(0.05)(GRU_layer)
            x = tf.keras.layers.ELU()(GRU_layer)
            ...
            early_stopping = EarlyStopping(monitor='val_mae', patience=18, restore_best_weights=True)
