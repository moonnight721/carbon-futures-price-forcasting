...
from tensorflow.keras.layers import SimpleRNN, Conv1D,concatenate, BatchNormalization,LeakyReLU,GlobalAveragePooling1D,Dropout,Input,Dense,Flatten,MaxPooling1D
...
for dataset_name, features in feature_sets.items():
    ...
    for t in range(start, end):
        ...
        for i in range(loop_times):
            ...
            rnn_layer = SimpleRNN(units,activation='tanh',return_sequences=False)(x)
            # dense=Dense(64)(rnn_layer)
            # x = tf.keras.layers.LeakyReLU(0.05)(rnn_layer)
            x = tf.keras.layers.ELU()(rnn_layer)
